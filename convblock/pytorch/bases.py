...
class MetaModule(type):
    ...
    def __mul__(cls, other):
        if not isinstance(other, int):
            raise TypeError
        return MulModule.p(module=cls, num_repeats=other)
    ...
    def __new__(mtcls, name, bases, attrs):  # noqa: N804
        attrs = {"_options": {}, "_instance_counter": 0, "_options_params": {}, **attrs}
    ...
    def __call__(cls, *args, **kwargs):
        if hasattr(cls, "_is_partial_module"):
            ...
            for key in keys & prev_keys:
                if isinstance(cls._partial_kwargs[key], dict) and isinstance(kwargs[key], dict):

                    new_kwargs[key] = {**cls._partial_kwargs[key], **kwargs[key]}
                else:
...
class Module(torch.nn.Module, metaclass=MetaModule):
    """Implementation of custom convblock framework module."""

    _input_shape: NDArray[np.int64]
    ...
    def __init__(self, input_shape: ArrayLike[int]):
        ...
        try:
            ...
            if input_shape.ndim > 2:
                raise ValueError("Argument 'input_shape'" + " must be 1d or 2d array")
        self._input_shape = np.array(input_shape, dtype=np.int64)
    ...
    def get_input_shape(self, input_id: int = 0) -> NDArray[np.int64]:
        ...
        """
        return self._input_shape[input_id, :]
    ...
    def count_parameters(self) -> int:
        """Count number of parameter requiring gradient computation.
    ...
    def num_inputs(self) -> int:
        ...
        """
        if self._input_shape.ndim == 1:
            return 1
        return int(self.input_shape.shape[0])
    ...
    def ndims(self) -> Literal[1, 2, 3]:
        ...
        -------
        Literal[1, 2, 3]
            number of spatial dimensions in input / output.
        """
    ...
    @property
    def input_shape(self) -> NDArray[np.uint64]:
        ...
        """
        return np.array(self._input_shape, dtype=np.uint64)

    @property
    def output_shape(self) -> NDArray[np.uint64]:
    ...
    def to_int_array(self, parameter: ArrayLike[int] | int, name: str, length: int):
        ...
        if isinstance(parameter, INT_TYPES):
            parameter = np.asarray([parameter] * length, dtype=np.int64)
        elif isinstance(parameter, LIST_TYPES):
            parameter = np.asarray(parameter, dtype=np.int64).flatten()
            if len(parameter) != length:
                raise ValueError(
                    "Argument {} has inproper lenght.".format(name)
                    + " Must have {}, got {}.".format(length, len(parameter))
                )
        else:
            raise ValueError(
    ...
    @classmethod
    def merge(cls, x: torch.Tensor, y: torch.Tensor, how: Literal["+", "*", "."] = "+"):
...
class Encoders(Module):
    ...
    def __init__(self, *modules):
        input_shapes = np.stack([m.input_shape for m in modules], axis=0)
        output_shapes = np.stack([m.output_shape for m in modules], axis=0)
        for i in range(len(input_shapes) - 1):
            if np.any(output_shapes[i] != input_shapes[i + 1]):
                raise ValueError(
                    "Input shape of each next module "
    ...
    def output_shape(self):
        output_shapes = np.stack([m.output_shape for m in self.layers], axis=0)
...
class Sum(Module):
    ...
    def __init__(self, encoder):
        ...
        if np.any(encoder.output_shape != out_shape):
            raise ValueError("Encoder outputs must" + " have equal output shapes")
...
class Cat(Module):
    ...
    def __init__(self, encoder):
        ...
        if np.any(encoder.output_shape[:, 1:] != out_shape[1:]):
            raise ValueError(
                "Encoder outputs must have equal " + " output shapes along spatial dimensions"
            )
        super().__init__(encoder.input_shape)
        self.layers = encoder
...
class Stack(Module):
    ...
    def __init__(self, *modules):
        input_shapes = np.stack([module.input_shape for module in modules], axis=0)
        super().__init__(input_shapes)
        self.layers = torch.nn.ModuleList(modules)

    @property
    def output_shape(self):
        return np.stack([module.output_shape for module in self.layers], axis=0)

    def forward(self, inputs):
        return [l(x) for l, x in zip(self.layers, inputs)]
...
class Pyramid(Module):
    ...
    def __init__(self, encoders, decoders, mode="+"):
        if mode not in "+*":
            raise ValueError("Argument 'mode' must be one of" + " following values: '+.*'.")
        for encoder, decoder in zip(encoders[::-1], decoders):
            if np.any(encoder.output_shape != decoder.input_shape):
                raise ValueError(
                    "Output shapes of encoders does not match" " input shapes of decoders"
                )
        input_shapes = np.stack([encoder.input_shape for encoder in encoders], axis=0)
        super().__init__(input_shapes)
    ...
    def output_shape(self):
        ...
        shape += [decoder.output_shape for decoder in self.decoders]
    ...
    def forward(self, inputs):
        ...
        outputs = [x]
        for decoder, y in zip(self.decoders, encoded[::-1]):
...
class ArithmeticModule(Module):
    ...
    def __init__(self, input_shape, op, value=None):
        ...
        if op not in allowed_values:
            raise ValueError(
                "Argument 'op' must be one " + "of following values: {}".format(allowed_values)
            )
        ...
        if isinstance(value, LIST_TYPES):
            ...
            value = torch.nn.Parameter(torch.from_numpy(value), requires_grad=False)
...
class Branches(Module):
    ...
    @classmethod
    def check_broadcast_shapes(cls, shapes: list, mode: str = ".", message: str = ""):
        ...
        if mode == ".":
            ...
            return np.concatenate(
                [
                    shapes[:, 0, None],
                    np.repeat(np.max(shapes[None, :, 1:], axis=1), repeats=shapes.shape[0], axis=0),
                ],
                axis=1,
            )
        else:
            if not np.all((shapes == shapes.max(axis=0)) | (shapes == 1)):
                raise ValueError(message)
            return np.repeat(np.max(shapes[None, ...], axis=1), repeats=shapes.shape[0], axis=0)
    ...
    def __init__(self, branches: list, mode="."):
        ...
        input_shapes = np.stack([branch.input_shape for branch in branches], axis=0)
        if np.any(input_shapes != input_shapes[0, :]):
            raise ValueError("All branches must have same input shape")

        input_strides = np.stack(
            [branch.stride for branch in branches if branch.stride is not None], axis=0
        )
        ...
        output_shapes = [
            np.concatenate(
                [
                    ...
                    branch.num_outputs
                    * [[1] * (_ndims - np.atleast_2d(branch.output_shape).shape[1])],
                ],
                axis=1,
            )
        ...
        if mode is None:
            ...
            output_shapes = self.check_broadcast_shapes(
                shapes=output_shapes, mode=mode, message=message
            )
            channels = np.sum(output_shapes[:, 0])
            self._output_shape = np.array([channels, *output_shapes[0, 1:]])
        else:
            message = (
                "All branches must have same output shape" " if 'mode' argument is set to '{mode}'"
            )
            output_shapes = self.check_broadcast_shapes(
                shapes=output_shapes, mode=mode, message=message
            )
    ...
    def _broadcast(a, b):
        if a.ndim == b.ndim:
            return a, b
        a_ = a.view(*[list(map(int, a.size())) + [1] * (max(a.ndim, b.ndim) - a.ndim)])
        b_ = b.view(*[list(map(int, b.size())) + [1] * (max(a.ndim, b.ndim) - b.ndim)])
        return a_, b_
    ...
    def forward(self, x: "Tensor") -> "Tensor":
        ...
        if self.mode == ".":
            return torch.cat(
                [u.expand(u.size(0), *self.output_shape.astype(int)) for u in outputs], dim=1
            )
        elif self.mode == "+":
            z = outputs[0]
            for y in outputs[1:]:
...
class Layer(Module):
    ...
    def __init__(self, input_shape: ArrayLike[int], layer: torch.nn.Module) -> None:
...
class Sequential(torch.nn.Sequential, metaclass=MetaModule):
    ...
    def stride(self) -> "Tuple[float]":
        ...
        Tuple[float] or None
            ...
            does not exist for current layer.
        """
        stride = np.ones(self.ndims - 1, dtype=np.float)
...
class ConvModule(Module):
    ...
    def __init__(self, input_shape, kernel_size=3, stride=1, dilation=1):
        ...
        self._kernel_size = transform_to_int_tuple(kernel_size, "kernel_size", self.ndims - 1)
        self._stride = transform_to_int_tuple(stride, "stride", self.ndims - 1)
        self._dilation = transform_to_int_tuple(dilation, "dilation", self.ndims - 1)
